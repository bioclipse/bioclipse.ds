...
<!--
  VERSION INFORMATION
   ...
   TODO:
      ...
      Perhaps replace schematron-full-path with a for-each implementation.
...
<xsl:template match="iso:schema[not(@queryBinding) or @queryBinding='xslt' 
	...
	<axsl:stylesheet version="1.0">
		<xsl:call-template name="stylesheetbody"/>
	</axsl:stylesheet>
</xsl:template>
...
<xsl:template name="handle-phase" >
	<xsl:if test="not($phase = '#ALL')">
	  <xsl:if test="not(iso:phase[@id = $phase])">
		...
		<axsl:if test="{@test}">
			...
			<xsl:call-template name="process-report">
				...
				<xsl:with-param name="diagnostics" select="@diagnostics"/>
					...
					<xsl:with-param name="role" select="@role" />
					<xsl:with-param name="subject" select="@subject" />
<!-- DP flag parameter missing. Added DP -->
                                        <xsl:with-param name="flag" select="@flag"/>
			</xsl:call-template>
   ...
   <xsl:template match="iso:include">
	...
	<xsl:template match="iso:pattern[not(@abstract='true')]">
      ...
      <xsl:if test="($phase = '#ALL') 
		...
		<!-- RJ. Replaced by following code from FG. Check.
		<axsl:template match="node() | @* | /" priority="-1" mode="M{count(preceding-sibling::*)}">
	...
	<!-- ISO TITLE -->
 <!-- DP, no class attribute on iso:title -->
...
<xsl:template name="handle-namespace">
       ...
       <xsl:choose>
        ...
        <xsl:when test="function-available('msxsl:node-set')">
          ...
          <xsl:copy-of select="msxsl:node-set($ns-dummy-elements)
                                 / *:dummy/namespace::*"/>
        </xsl:when>
        -->
...
<!-- ============================================================== -->
   ...
   <xsl:message terminate='no'>
     skel: title context is <xsl:value-of select='name()'/>
   </xsl:message>
