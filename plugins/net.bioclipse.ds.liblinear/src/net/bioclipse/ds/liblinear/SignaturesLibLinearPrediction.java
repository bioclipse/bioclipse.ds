 ...
 *
 */
public class SignaturesLibLinearPrediction extends AbstractDSTest{


	//The logger of the class
	private static final Logger logger = Logger.getLogger(SignaturesLibLinearPrediction.class);
	...
	public SignLibLinearModel signLibLinearModel;
...
//		ret.add( SIGNATURES_MIN_HEIGHT );
	...
	public void initialize(IProgressMonitor monitor) throws DSException {
		...
		logger.debug("Initializing liblinear test: " + getName());
...
//		}
		...
		try {
			...
			signLibLinearModel = new SignLibLinearModel(model, modelSignatures);
		} catch (IOException e) {
			throw new DSException("Could not read model file '" + modelPath 
					+ "' due to: " + e.getMessage());
		}

		logger.debug("Initializing of liblinear test: " + getName() 
	...
	protected List<? extends ITestResult> doRunTest(ICDKMolecule cdkmol,
		...
		return predictLibLinearSVM(cdkmol,false,monitor);
	 ...
	 * @return
	 */
	public List<? extends ITestResult> predictLibLinearSVM(ICDKMolecule cdkmol,
			boolean extractSignificantSignatures, 
			IProgressMonitor monitor) {
		...
		Iterator<String> signaturesIter = signLibLinearModel.getModelSignatures().iterator();
		while (signaturesIter.hasNext()){
			...
			if (predModel.getMoleculeSignatures().containsKey(currentSignature)){
				int index = signLibLinearModel.getModelSignatures().indexOf(currentSignature)+1; // libsvm assumes that the index starts at one.
				Double value = (Double) predModel.getMoleculeSignatures().get(currentSignature);
...
//		//************************
//		svm_model svmModel = signLibLinearModel.getSvmModel();  //Get the SVM model from the wrapper object
//		
//		// Get the most significant signature for classification or the sum of all gradient components for regression.
//		List<Double> gradientComponents = new ArrayList<Double>();
...
//					}
//					if (maxComponent > 0.0){
//						System.out.println("Max atom: " + predModel.getMoleculeSignaturesAtomNr().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMaxVal].index-1)) + ", max val: " + gradientComponents.get(elementMaxVal) + ", signature: " + signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMaxVal].index-1) + ", height: " + predModel.getMoleculeSignaturesHeight().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMaxVal].index-1)));
//
//						significantSignature=signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMaxVal].index-1);
//						height=predModel.getMoleculeSignaturesHeight().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMaxVal].index-1));
//						centerAtoms=predModel.getMoleculeSignaturesAtomNr().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMaxVal].index-1));
//
//					}
//					else{
...
//					}
//					if (minComponent < 0.0){
//						System.out.println("Min atom: " + predModel.getMoleculeSignaturesAtomNr().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMinVal].index-1)) + ", min val: " + gradientComponents.get(elementMinVal) + ", signature: " + signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMinVal].index-1) + ", height: " + predModel.getMoleculeSignaturesHeight().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMinVal].index-1)));
//
//						significantSignature=signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMinVal].index-1);
//						height=predModel.getMoleculeSignaturesHeight().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMinVal].index-1));
//						centerAtoms=predModel.getMoleculeSignaturesAtomNr().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[elementMinVal].index-1));
//
//					}
//					else{
//						System.out.println("No significant signature.");
...
//			for (int element = 0; element < svmPredictionArray.length; element++){
//				double componentVal = gradientComponents.get(element);
//				List<Integer> atomNrList = predModel.getMoleculeSignaturesAtomNr().get(signLibLinearModel.getModelSignatures().get(svmPredictionArray[element].index-1));
//				Iterator<Integer> atomNrInteger = atomNrList.iterator();
//				while (atomNrInteger.hasNext()){
//					int atomNr = atomNrInteger.next();
//					if (atomGreadientComponents.containsKey(atomNr)){
	...
	private PredictionModel createPredModel(ICDKMolecule cdkmol) throws BioclipseException {
		...
		for (int height = startHeight; height <= endHeight; height++){
			...
			while (signsIter.hasNext()){
				...
				if (signLibLinearModel.getModelSignatures().contains(currentSignature)){
